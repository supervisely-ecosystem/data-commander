    ...
    def _create_rec(
        ...
        if dataset_info is not None:
            ...
            if created_info is None:
                raise RuntimeError(
                    f"Failed to get info for dataset {created_id}. "
                    f"Dataset may have been deleted or access was denied."
                )
...
def get_item_infos(
    dataset_id: int, item_ids: List[int] = None, project_type: str = sly.ProjectType.IMAGES
):
    filters = None
...
def merge_items(
    ...
    for task in as_completed(tasks):
        ...
        src_dataset_info = next(
            (ds for ds in src_datasets if ds.id == this_dataset_item_infos[0].dataset_id), None
        )
        dst_dataset_info = get_or_create_dataset_in_dst(
            src_dataset_info,
